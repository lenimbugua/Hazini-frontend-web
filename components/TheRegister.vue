...
<template>
  <form @submit.prevent="register" class="w-full max-w-sm">
    <div class="mb-6">
      <label for="full_names" class="block text-sm font-semibold leading-6 text-gray-900">Username</label><input
        type="email" name="full_names" id="full_names" placeholder="username@example.com"
    ...
    <div class="mb-6">
      <label for="full_names" class="block text-sm font-semibold leading-6 text-gray-900">Full Names</label><input
        type="text" name="full_names" id="full_names" placeholder="James Doe"
        class="mt-2 appearance-none text-slate-900 bg-white rounded-md block w-full px-3 h-10 shadow-sm sm:text-sm focus:outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-[#37899A] ring-1 ring-slate-200"
    ...
    <div class="mb-6">
      ...
      <p v-if="error.hasError && error.field === 'phone_number'" class="text-red-500 mb-6 text-sm">
        {{ error.message }}
      </p>
    </div>
    <div class="mb-6">
      <label for="password" class="block text-sm font-semibold leading-6 text-gray-900">Password</label><input
        type="password" name="password" id="password" placeholder="xxxxxxxx"
        class="mt-2 appearance-none text-slate-900 bg-white rounded-md block w-full px-3 h-10 shadow-sm sm:text-sm focus:outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-[#37899A] ring-1 ring-slate-200"
        aria-required required v-model="password" minlength="6" />
    </div>
    <p v-if="error.hasError && error.field === 'none'" class="text-red-500 mb-6">
      {{ error.message }}
    </p>
    <button type="submit"
